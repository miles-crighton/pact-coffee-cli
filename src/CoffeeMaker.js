...
class CoffeeMaker {
  ...
  displayOrderStatus = async () => {
    try {
      if (!this.userData) {
        throw new Error('No user data found, try calling getUserData()');
      }
      const coffeeName = this.userData.items[0]['product_name'];
      const dispatchDate = this.userData.orders[0]['dispatch_on'];
      const daysTo = moment(dispatchDate, 'YYYY-MM-DD').diff(moment(), 'days');

      console.log(
        chalk.yellow('Your order of'),
        chalk.green(coffeeName),
        chalk.yellow(`☕ will be dispatched in`),
        chalk.red(`${daysTo} days`),
        chalk.white(`(${helpers.reverseDate(dispatchDate, '-', '/')})`)
      );
    } catch (e) {
  ...
  getOrderHistory = async () => {
    try {
      if (!this.authToken) {
        throw new Error('No auth token found, try calling authenticate()');
      }
      this.orderHistory = await APIInterface.getOrderHistory(this.authToken);
    } catch (e) {
  ...
  displayOrderHistory = async () => {
    try {
      ...
      if (orderHistory) {
        ...
        console.log(
          chalk.white.bold(`Last ${slicedOrderHistory.length} orders:`)
        );
        ...
        for (let order of slicedOrderHistory) {
          ...
          if (coffeeItem) {
            ...
            console.log(
              ...
              orderStatus === 'shipped'
                ? chalk.green(`(Status: ${orderStatus.toUpperCase()} ✅)`)
                : chalk.red(`(Status: ${orderStatus.toUpperCase()} ❌)`)
            );
          }
        }
      }
